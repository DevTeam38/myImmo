;( function( window ) {
    ...
    function getRadius( element, position ) {
        var w = Math.max( position.x, (element.clientWidth)/10 - position.x );
        var h = Math.max( position.y, (element.clientHeight)/10 - position.y );
        return Math.ceil( Math.sqrt( Math.pow( w, 1.3 ) + Math.pow( h, 1.3 ) ) ); //permet d'agrandir ou réduir le cercle
    }
